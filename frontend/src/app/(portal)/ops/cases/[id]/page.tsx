...
interface CaseDetail {
  ...
  tasks: CaseTask[];
  // docs/AUTOMATION_PRICING_ENGINE_SCOPE.md Phase 4 — present only when
  // this case came from a ServiceRequest (absent for recurring-schedule
  // spawns); null outcome means that request never reached an
  // automation decision (or automation wasn't enabled for its service
  // type at the time).
  originRequest: { automationDecision: { outcome: string } | null } | null;
}
  ...
  return (
    <div>
      <div className="hero">
        ...
        {detail.originRequest?.automationDecision?.outcome === 'AUTO' && (
          <span className="badge" title="Created automatically — no staff member triaged this request. If a quote already exists, check whether it was auto-generated by the pricing engine or added by staff.">
            🤖 Automated
          </span>
        )}{' '}
