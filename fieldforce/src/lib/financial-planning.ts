...
// ═══════════════════════════════════════════════════════════════════════════════
// Turns an agent's real WalletEntry history (MISSION_EARNING rows — the same
// ledger postEarnings()/ledger.ts writes on every completed, QC-cleared
// mission) into three things field agents in the gig economy consistently
// lack: a forward-looking earnings projection, a savings plan, and a plain-
// language Nigerian PAYE tax estimate. All figures are whole Naira, matching
// this codebase's existing convention (ledger.ts's own descriptions display
// amounts with no /100 conversion — WalletEntry.amount is Naira, not kobo).
//
// Tax estimate caveat, the same "confirmed vs not" honesty this codebase
// already applies elsewhere (voice-translation.ts's Whisper-accuracy note,
// ZAVU_INBOUND_SIGNATURE_OUTREACH.md): the band table below is Nigeria's
...
/** Buckets this agent's MISSION_EARNING wallet entries by calendar month,
 * oldest first. Only counts entries actually posted to the ledger — never
...
export async function getMonthlyEarningsHistory(
  ...
  const entries = await db.walletEntry.findMany({
    where: { agentId, type: 'MISSION_EARNING', createdAt: { gte: since } },
    select: { amount: true, createdAt: true },
    orderBy: { createdAt: 'asc' },
  });

  const byMonth = new Map<string, { totalNaira: number; missionCount: number }>();
  for (const entry of entries) {
    const key = `${entry.createdAt.getFullYear()}-${String(entry.createdAt.getMonth() + 1).padStart(2, '0')}`;
    const bucket = byMonth.get(key) ?? { totalNaira: 0, missionCount: 0 };
    bucket.totalNaira += entry.amount;
...
export async function getFinancialPlan(agentId: string, monthsBack = 6): Promise<FinancialPlan> {
  const history = await getMonthlyEarningsHistory(agentId, monthsBack);
